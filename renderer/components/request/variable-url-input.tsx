...
export function VariableUrlInput({
  ...
  // Combine user environment variables and built-in dynamic variables
  ...
  // Resolution status for live preview
  ...
  const handleSelectSuggestion = (suggestionKey: string) => {
    ...
    if (lastOpenBraces !== -1) {
      ...
      // Check if closing braces already exist right after cursor
    ...
    <div className={`relative flex-1 flex flex-col ${className}`}>
      ...
      <div className="relative flex items-center w-full">
        <Input
          ...
          disabled={disabled}
          className="h-9 pr-8 text-xs font-mono bg-card/60 border-border focus-visible:ring-[#0275E2]/40"
        />

        {/* Variables quick menu button */}
        <Popover>
          <PopoverTrigger asChild>
            <button
              type="button"
              className="absolute right-2 text-muted-foreground/60 hover:text-[#0275E2] p-1 rounded transition-colors"
              title="Browse & Insert Variables (type '{{' anywhere)"
            >
              <Braces className="size-3.5" />
            </button>
          </PopoverTrigger>
          <PopoverContent align="end" className="w-72 p-2 text-xs font-sans">
            ...
            <div className="max-h-56 overflow-y-auto pt-1 space-y-1 scrollbar-none">
              ...
              {currentVariables.length > 0 ? (
                currentVariables.map((v) => (
                  ...
                  >
                    ...
                    <span className="text-[11px] text-muted-foreground truncate max-w-[120px] font-mono">
                      {v.value}
                    </span>
                  </button>
                ))
                ...
                <div className="px-2 py-1.5 text-[11px] text-muted-foreground italic">
                  No variables set in active environment.
                </div>
              )}

              {/* Dynamic Built-in Variables */}
              <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider px-1 pt-2 border-t border-border/40">
                Dynamic Variables
              </div>
              {BUILT_IN_VARIABLES.map((b) => (
                <button
                  ...
                  className="w-full flex items-center justify-between px-2 py-1 rounded-md text-left hover:bg-muted/50 transition-colors group"
                >
                  <div className="flex items-center gap-1.5 font-mono text-xs text-amber-500 font-medium">
      ...
      {showAutocomplete && filteredSuggestions.length > 0 && (
        <div className="absolute top-10 left-0 z-50 w-72 rounded-md border border-border bg-popover p-1 text-popover-foreground shadow-xl animate-in fade-in-0 zoom-in-95">
          <div className="px-2 py-1 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider border-b border-border/50 flex items-center justify-between">
            <span>Insert Variable</span>
            <span className="text-[9px] font-normal">↑↓ navigate, Enter select</span>
          </div>
          <div className="max-h-48 overflow-y-auto py-1 scrollbar-none space-y-0.5">
              ...
              >
                <div className="flex items-center gap-1.5 truncate">
                  {s.isBuiltIn ? (
                    ...
                    <Zap className="size-3 shrink-0 text-[#0275E2]" />
                  )}
                  <span className="font-mono">{`{{${s.key}}}`}</span>
                </div>
                <span
                  className={`text-[10px] truncate max-w-[100px] font-mono ${
      ...
      {/* Live Variable Resolution Status / Preview Bar */}
      {status.hasVariables && (
