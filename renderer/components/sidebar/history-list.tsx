...
export function HistoryList({ onCountChange, clearTrigger }: HistoryListProps = {}) {
  ...
  const handleRestore = (item: HistoryItem) => {
    const restored = createDefaultRequest({
      ...
      url: item.url,
      headers: Object.entries(item.request_snapshot.headers || {}).map(([k, v]) => ({
      ...
      body: item.request_snapshot.body
        ? {
            type: typeof item.request_snapshot.body === 'string' ? 'raw' : 'json',
            raw:
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
          ...
          >
            <div className="flex items-center justify-between gap-1.5">
              <div className="flex items-center gap-2 truncate">
                <MethodBadge method={item.method} protocol={item.protocol} size="sm" />
                <span className="text-xs font-mono truncate text-foreground/90">{item.url}</span>
              </div>
              {item.status && (
                <span
                  className={cn(
            ...
            <div className="flex items-center justify-between text-[10px] text-muted-foreground font-mono">
              <div className="flex items-center gap-2">
                {item.duration_ms !== null && (
                  <span className="text-foreground/70">{item.duration_ms} ms</span>
                )}
        ...
        {historyItems.length === 0 && !isLoading && (
          <div className="flex flex-col items-center justify-center h-48 text-muted-foreground/60 text-xs text-center px-4">
            ...
            <p className="font-medium text-foreground/70">No execution history yet</p>
            <p className="text-[11px] mt-0.5">Sent requests and responses will be logged here.</p>
          </div>
        )}
      </div>
    </div>
